...
Stars in isolation prove nothing. Stars + downloads + issues, *together*, are the real signal.
...
|---|---|---|
| **Active OSS projects** (T1 + T2 + active T3) | Portfolio breadth | 5–10. Below 5: thin portfolio. Above 10: focus problem. |
...
| **Sum of new external contributors across portfolio** | Audience-formation pace | Higher = healthier. A quarter with zero is a flag. |
| **Launches executed per quarter** | Cadence health | 1–3 per quarter. Zero = not shipping. >3 = scattered. |
...
| Cadence | What | Where |
|---|---|---|
| **Weekly (Monday)** | Status check on T2 projects + ASKS.md cleanup | Update each `projects/<name>.md` "Status" section + `_index.md` |
...
| **Quarterly** | Tier reassessment + policy revision | Edit `policy.md` §1, update `_index.md` |
...
When the portfolio matures (multiple T1 projects, recurring T2 launches landing every quarter), this document gets rewritten. The current shape is a 6–12 month operating contract, not the permanent regime.
