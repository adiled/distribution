...
**Effort tiers.** `S` (set-and-forget, < 1 hr to set up), `M` (recurring author work, 1 to 4 hr per use), `L` (named launch beat, 4+ hr including prep and day-of presence), `XL` (talk / interview / longform, days of prep).
...
| **`awesome-claude-code`** | M | ★★ | Curated, growing audience. Submission requires basic fit and a working repo. |
| **`awesome-opencode`** (already a fork in your repos) | M | ★★ | Smaller list, but `opencode-dir` and `clwnd` belong here on day one. |
| **`awesome-rust`** subsections | M | ★★ | For `orch`/`orchd`/`ccft`. Service-orchestration and agent-tooling subsections both apply. |
...
| Channel | Effort | Yield | Notes |
|---|---|---|---|
| **Show HN** | L | ★★★★ | The biggest single beat available. Unforgiving. Broken install on launch day equals lost quarter. T2-ready projects only. Ideal: Tuesday to Thursday, 8 to 10am PT post. |
...
| **Mastodon (fediverse and tech instances)** | M | ★ | Low yield but the audience that does engage is high-quality. |
...
| **Personal blog (`adils.me`)** | L | ★★★ | The single highest-yield owned channel. Each piece is permanent and SEO-able. Use for thesis posts, postmortems, and "why we built X" pieces. |
...
| **Latent Space (newsletter and podcast)** | XL | ★★★★ | High-quality AI engineering audience | Editorial. Landing requires either being interviewed or writing a guest piece. |
...
| Channel | Effort | Yield | Notes |
|---|---|---|---|
| **Per-project landing page** | L (one-time) | ★★★ | Vercel/Cloudflare Pages plus a simple Next.js or static page. Outranks GitHub for the project name in search. Collects email signups. Mandatory before T2 launch. |
...
| **Topics and description on every repo** | S | ★★ | Free reach via GitHub search and topic pages. Currently underused. Most repos have empty descriptions. |
...
### `ohlc-resample` (T1)
**Maintain.** npm publishing pipeline plus Changesets is already solid. Promote on awesome-algotrading lists. Stop active campaigning. The asset is the long tail. Add to `adils.me` portfolio above the fold.

### `clwnd` (T2)
Coordinated launch: opencode plugin marketplace post, `awesome-opencode` PR, Show HN, `/r/LocalLLaMA`, and `adils.me` thesis post. Demo gif (already exists) is the lede everywhere.

### `opencode-dir` (T2)
Lighter coordinated launch: `awesome-opencode` PR, opencode community channels, and an X thread with `/cd` demo. No Show HN. The audience is too narrow, and HN flames niche tools.
...
### `orch` / `orchd` (T3)
Position as **alternative to docker-compose for local services**. That framing gives a search-friendly anchor. crates.io publish, `awesome-rust`, `awesome-self-hosted`, and a comparison post on `adils.me`. Talk submissions to RustConf and FOSDEM down the line.
