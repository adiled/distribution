...
A sentinel for multiplexed cross-harness AI bots. Manages concurrent Claude Code processes through opencode, with permission gating, idle-process killing, MCP proxying, and an optional drone-style observer for context-loss detection and auto-recovery. Daemon-shaped: `clwnd update`, `clwnd status`, `clwnd logs`, `clwnd sessions`. Has a working install script and an existing demo gif in the repo.
...
- ✅ Config schema documented (table of keys and defaults).
...
2. **README front-matter.** Restructure to 1-line pitch, then demo gif, then "what this solves" paragraph, then install snippet, then quick example. The current order assumes domain familiarity, this order doesn't.
...
5. **Homebrew tap.** `adiled/tap` repo with a `Formula/clwnd.rb`. About 1 hour of work.
6. **Issue templates.** Bug-report and feature-request templates so first-contact visitors don't have to invent the shape.
7. **Landing page (optional but strongly recommended).** Single-page Vercel deploy: hero, demo gif, install snippet, "who built this" link to `adils.me`. About 2 hours of work.
8. **`adils.me` thesis post.** Title direction: *"clwnd: governing fleets of coding agents without losing your mind."* The post is the primary artifact of the launch. Show HN, Reddit, and X all link to it, not to the repo.

**Launch day (Tuesday or Wednesday morning, US Pacific):**

1. Publish the `adils.me` post.
2. Open Show HN with the post URL (not the repo URL, since the post explains the why).
...
- Run a second light beat about 2 weeks after if the first one underperforms. `Console.dev` newsletter submission, Latent Space tip line, podcast outreach.
