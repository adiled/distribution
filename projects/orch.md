...
**Tier:** T3 (Ambitious / pre-launch) • **Repos:** [`orch`](https://github.com/adiled/orch) (parser/spec) and [`orchd`](https://github.com/adiled/orchd) (execution engine) • **License:** *not declared in README* (verify) • **Stars:** 0 + 0

## What they are

A two-repo Rust toolchain for **declarative, platform-agnostic service orchestration**:

- **`orch`.** Parser and spec for *Orchfiles*. Reads an Orchfile, validates constraints, expands variables, detects dependency cycles, emits structured JSON. Multi-file composition with systemd-drop-in semantics (last-wins scalars, key-merge lists, append-and-dedup positional, CLEAR directive). ARG overrides via CLI flag or env var.

- **`orchd`.** Execution engine. Consumes orch's parsed JSON and produces native service-manager artifacts. Runtime layer (`bare` shipping; `containerd`, `podman`, `apple` planned) plus Platform layer (`systemd` shipping; `launchd` planned). `orchd up`, `down`, `restart`, `status`, `logs --follow`, `health --timeout`, `list`, `clean`. Configurable via `.orchrc` (project or `$HOME`), env, or CLI flags with documented merge order.

Pipeline: **Orchfile, then orch parse (JSON), then orchd Runtime (ExecSet), then Platform (native artifacts).**
...
Distribution-wise, it is also the project most likely to land in **Rust and self-hosting communities** as opposed to the agent-systems audiences the rest of the portfolio targets. That diversification is itself valuable.

## Audience

- **Self-hosters** running services on a personal server or homelab who find `docker-compose` over-engineered and `systemd` units verbose.
- **Rust users** looking for tasteful tools written in Rust.
- **Developers who want service orchestration without containers.** orch's `bare` runtime plus systemd platform is the killer combo here.
...
**Phase 3: Distill (weeks 4 to 6):**

8. **`adils.me` essay.** *"Orch: declarative service orchestration that targets your platform, not Docker."* The essay positions the project against the alternatives, shows the postgres-and-app example end-to-end, and explains the Runtime/Platform split as a deliberate choice (not a TODO).
9. **Demo recording.** About 60 seconds of asciinema: write Orchfile, then `orchd generate`, then `orchd up`, then `orchd status`, then `orchd logs --follow`.
...
When (a) both crates published, (b) essay published, (c) demo recorded, and (d) at least one launchd platform commit landing, the project graduates to T2 (which at this point will be largely retrospective).
...
- **Not adding more runtimes (containerd, podman, apple) before the bare runtime is well-tested.** Premature breadth dilutes the argument that bare-runtime plus systemd is a deliberate position.
...
*(None yet. Blocked on license, crates publish, and comparison framing.)*

## Notes

orch/orchd is the project where **clear technical framing beats novelty.** The Runtime/Platform split is the deliberate technical contribution. The cleanest way to communicate it is *one good example program that does something familiar* (run a postgres-and-app stack) and *one direct comparison block* explaining what's different. Resist the temptation to oversell. The audience for service orchestration is technical and skeptical, and oversold copy reads as a red flag.
