...
A TypeScript library and CLI that resamples market data (OHLCV candles, raw trades, ticks) between time frames. CCXT-compatible. Single dependency. Has its own CLI (`ohlc-resample -i in.csv -b 60 -n 300`), supports JSON and CSV, supports pipe input, supports two equivalent JSON shapes (object and CCXT-tuple).
...
- Quantitative traders and algotrading hobbyists building backtest pipelines.
...
3. **Week 3.** Add `ohlc-resample` to the `adils.me` portfolio above the fold, with one-line pitch and npm badge.
...
- **No rewriting it in Rust to "modernize."** The asset *is* the stable, working, npm-published TypeScript library that just keeps working. Rewrite is not improvement.
