...
- **Per-session isolation** via branches and vessels. No cross-contamination.
...
Install: `curl -fsSL ...iksir/main/install | bash` (after Deno and opencode are present).
...
The cultural register (Arabic alchemical vocabulary, Sufi-adjacent framing) is also distinctive in a way the rest of the public portfolio is not.
...
- The "single-piece-flow" and lean-software audience. This is a small but engaged niche.
- Practitioners drawn to mythic or alchemical framings as productivity / discipline anchors (small but devoted audience: see Tools for Thought, Hyperland, etc.).

## Distribution state: what's done

- ✅ MIT license declared.
- ✅ Working install script with documented update and uninstall paths.
...
- ❌ Telegram and Linear defaults are presented as if every user has them. State the prerequisites explicitly and document substitutes.

## Plan: pre-launch

**Phase 1: Add the operational layer (weeks 1 to 3):**
...
2. **Record the demo.** About 60 to 90 seconds. Submit a recipe, see worker pick it up, watch it block on a question, principal answers via Telegram, worker resumes, completion notice.
...
When operational README, demo, and essay are public, and at least one external user has installed and reported back, the project graduates to T2 with the launch checklist.
...
T3 readiness gates from `metrics.md`. Project-specific watchpoint: **whether the operational README addition produces inbound from cold-readers.** A measurable change in install-script-hits before and after the operational tier is added is the cleanest signal that the README change worked.

## Launches

*(None yet. Blocked on operational README and demo.)*
